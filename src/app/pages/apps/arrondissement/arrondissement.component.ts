...
import { Arrondissement, ArrondissementService } from '../../../services/arrondissement.service';
...
@Component({
  ...
  styleUrl: './arrondissement.component.css'
})
export class ArrondissementComponent {
  ...
  ngOnInit(): void {
    //appelle de la methode de recuperation des arrond...
  ...
  getAllArrondissement() {
    console.log("_________________dans getAllArrondissement");
    
    this.arrondissementService.getArrondissements().pipe(
      retry(this.nbFois),
      catchError(error => {
        console.log("Erreur lors de la récupération des Arrondissements", error);
        return throwError("Veuillez réessayer");
      })
    ).subscribe(response => {
      this.listeArrondissement = response; 
      console.log("**********************", this.listeArrondissement);
    });
  }
